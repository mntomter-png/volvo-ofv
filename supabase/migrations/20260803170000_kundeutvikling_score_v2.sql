-- Kundeutvikling v2: mer differensiert score + status (byttet merke / ingen kjøp / redusert).
-- Score basert på absolutte tapte enheter + konkurrentkjøp, ikke bare %-fall til null.
...
as $$
  ...
  scored as (
    select
      b.*,
      (b.prior_focus - b.current_focus)::int as lost_units,
      ...
      case
        ...
        when b.current_total = 0 then 'dormant'
        else 'reduced'
      end as status,
      -- Absolutte tapte enheter (1→8, 6+→45). Skiller store fra små kontoer.
      least(45, (b.prior_focus - b.current_focus) * 8)::int as volume_score,
      -- Konkurrentkjøp i perioden (høyest prioritet for selger).
      case
        when b.current_total > b.current_focus
          then least(35, 15 + (b.current_total - b.current_focus) * 5)
        when b.current_focus > 0 and b.current_focus < b.prior_focus
          then 10  -- fortsatt hos merket, men redusert
        else 0     -- ingen kjøp i det hele tatt
      end::int as share_score,
      -- Tid siden siste fokusmerke-kjøp (moderat vekt).
      round(
        least(
          20,
          greatest(
  ...
  select
    ...
    case
      ...
      when current_share_pct is null then round(0 - prior_share_pct, 1)
      else round(current_share_pct - prior_share_pct, 1)
    end as share_delta_pp,
    competitor_units,
    status,
    (volume_score + share_score + recency_score)::int as priority_score,
    volume_score,
    share_score,
    recency_score
  from scored
  order by
    (volume_score + share_score + recency_score) desc,
    lost_units desc,
