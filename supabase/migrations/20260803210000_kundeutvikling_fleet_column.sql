-- Kundeutvikling: vis faktisk flåte fra populasjon i stedet for «Volvo 10 år».
-- Kundebase (≥2 kjøp / 10 år) uendret; size_score baseres på Volvo-flåte nå.
...
begin
  for r in
    ...
    where pronamespace = 'public'::regnamespace
      and proname = 'reg_owner_focus_decline_list'
  loop
...
as $$
  ...
  periods as (
    select
      ...
      (cur_end - interval '10 years')::date::timestamp as lookback_start,
  ...
  scored as (
    select
      ...
      -- Størrelse etter faktisk Volvo-flåte (maks 25).
...
grant execute on function public.reg_owner_focus_decline_list(
  int, text, int, int, text, text, int, text, int, date, date, int, boolean, text, text
) to authenticated;
