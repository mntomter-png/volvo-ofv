-- Kundeutvikling v4: 10-års lookback så «Forfalt» (≥5 år) fungerer.
-- Status: kun_konkurrent | også_konkurrent | forfaller | forfalt.
...
as $$
  ...
  scored as (
    select
      ...
      least(25, b.focus_10y * 2)::int as size_score,
      case
        when b.current_total > b.current_focus and b.current_focus = 0
  ...
  select
    ...
    recency_score
  from scored
  where status in ('competitor', 'mixed', 'due', 'overdue')
  order by
    case status
