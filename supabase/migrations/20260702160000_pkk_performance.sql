-- PKK: raskere spørringer — én passering, bedre indekser, lett summary.

create index if not exists population_snapshot_make_heavy_idx
  on public.population (snapshot_date, make_name)
  where maximum_laden_mass_kg >= 16000;

create index if not exists population_snapshot_has_pkk_idx
  on public.population (snapshot_date)
...
as $$
  ...
  vehicles as (
    select
      ...
      p.primary_owner_name,
      p.primary_owner_orgnr,
      coalesce(
