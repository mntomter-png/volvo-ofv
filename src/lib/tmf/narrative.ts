...
export function buildTmfNarrative(estimate: TmfEstimateResult): TmfNarrative {
  ...
  const lead = `${nextYear.year} er estimert til ${formatNumber(Math.round(nextMarket))} nyregistreringer (P50), ${signedPct(deltaPct)} mot prognosen for ${currentYear.year} (${formatNumber(Math.round(currentMarket))}). Volvo-estimatet er ${formatNumber(Math.round(nextYear.total.annualVolvo))} (${formatPercent(nextYear.total.volvoSharePct, 1)} % andel). Drivlinje: EMOB ${formatPercent(nextYear.total.emobSharePct, 1)} % (${formatNumber(Math.round(nextYear.total.annualEmob))}), diesel ${formatNumber(Math.round(nextYear.total.annualDiesel))} og gass ${formatNumber(Math.round(nextYear.total.annualGas))}. Usikkerhetsbåndet ligger på ${formatNumber(Math.round(confidence.market.p10))}–${formatNumber(Math.round(confidence.market.p90))}.`;
  ...
  const caveats = [
    ...
    "EMOB/diesel/gass er mekanisk split av TMF-volumet med trailing 12-mnd andel (samme mønster som Volvo-andel) — ikke egne drivlinjeprognoser.",
  ];
